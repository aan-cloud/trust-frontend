...
export const Card: any = ({ product }: productsType) => {
  return (
    <Link to={`/${product.category}/${product.slug}`}>
      <div key={product.id} className="text-white bg-[hsl(240deg,3.7%,15.9%,50%)] flex flex-col justify-between items-center gap-2 rounded-sm hover:scale-105 transition-transform duration-300 ease-in-out shadow-sm">
      ...
      <div className="flex flex-col pb-4 pl-4 pr-4 justify-start w-full">
        ...
        <p className="font-poppins font-extralight text-[11px]">Stock {product.stock}</p>
